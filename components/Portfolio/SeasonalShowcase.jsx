...
export default function SeasonalShowcase() {
  ...
  return <section className={"seasonal-showcase season-" + theme.theme_code} style={{ "--season-accent": theme.accent, "--season-bg": theme.background }} aria-label={seasonLabels[theme.theme_code] || t("season.studio")}><div className="seasonal-layout"><div className="seasonal-symbol" aria-hidden="true"><span className="season-orb"/><span className="season-crescent"><FaMoon/></span><span className="season-star season-star-one"><FaStar/></span><span className="season-star season-star-two"><FaStar/></span><span className="season-gift"><FaGift/></span></div><div className="seasonal-copy"><span className="seasonal-label"><FaWandMagicSparkles/> {seasonLabels[theme.theme_code] || t("season.studio")}</span><h2>{theme.headline}</h2><p>{theme.message}</p><a href="#work">{t("season.explore")} <FaArrowUpRightFromSquare/></a></div></div><div className="seasonal-lines" aria-hidden="true"/></section>;
}
