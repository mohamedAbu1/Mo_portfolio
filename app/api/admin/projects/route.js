...
export async function PUT(req) {
  if (!await guard()) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  try { const body = await req.json(); const engagementId = Number(body.engagementId); const title = String(body.title || "").trim(); const items = Array.isArray(body.deliverables) ? body.deliverables : []; if (!engagementId || title.length < 2 || !items.length) return NextResponse.json({ error: "Project, title and at least one deliverable are required" }, { status: 400 }); const existing = await query("SELECT id FROM engagements WHERE id=? LIMIT 1", [engagementId]); if (!existing.length) return NextResponse.json({ error: "Project not found" }, { status: 404 }); await query("UPDATE engagements SET title=?,description=?,status=? WHERE id=?", [title, clean(body.description), body.status || "planned", engagementId]); for (const item of items) await saveDeliverable(engagementId, item, item.id ? Number(item.id) : null); return NextResponse.json({ id: engagementId, updated: true }); } catch (error) { console.error("Update project failed", error); return NextResponse.json({ error: "Unable to update project" }, { status: 500 }); }
}
