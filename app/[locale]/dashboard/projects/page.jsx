...
function Fields({ item, index, update, fileUrl }) {
  ...
  return <div className="deliverable-fields">
    ...
    <label className="file-field">{doc ? "Document preview images" : "Cover image"}<input type="file" accept="image/*" multiple={doc} onChange={(e) => update(index, doc ? "galleryFiles" : "coverFile", doc ? Array.from(e.target.files || []) : e.target.files?.[0] || null)} />{doc && item.galleryFiles.length > 0 && <small className="file-count">{item.galleryFiles.length} new images selected</small>}{!doc && item.coverFile && <img className="file-preview" src={fileUrl(item.coverFile)} alt="New cover preview" />}{!doc && !item.coverFile && !item.removeCover && item.coverImageUrl && <><img className="file-preview" src={item.coverImageUrl} alt="Current cover" /><button type="button" className="file-remove-button" onClick={() => { update(index, "removeCover", true); update(index, "coverImageUrl", ""); }}>Remove current cover</button></>}</label>
    {!doc && <><label className="file-field">Gallery images<input type="file" multiple accept="image/*" onChange={(e) => update(index, "galleryFiles", Array.from(e.target.files || []))} />{item.galleryFiles.length > 0 && <small className="file-count">{item.galleryFiles.length} new images selected</small>}{item.imageUrls.length > 0 && <><small className="file-count">{item.imageUrls.length} existing images kept</small><div className="existing-image-list">{item.imageUrls.map((url) => <span className="existing-image-item" key={url}><img src={url} alt="Existing gallery image" /><button type="button" onClick={() => { update(index, "imageUrls", item.imageUrls.filter((image) => image !== url)); update(index, "removedImageUrls", [...item.removedImageUrls, url]); }} aria-label="Remove gallery image">×</button></span>)}</div></>}</label><label className="file-field">Short video<input type="file" accept="video/mp4,video/webm,video/quicktime" onChange={(e) => update(index, "videoFile", e.target.files?.[0] || null)} />{item.videoFile && <small className="file-count">{item.videoFile.name}</small>}</label></>}
  ...
  async function save(event) {
    ...
    try {
      ...
      for (const item of form.deliverables) {
        ...
        items.push({ ...item, coverImageUrl: item.coverFile ? await upload(item.coverFile, "image") : item.removeCover ? "" : item.coverImageUrl, imageUrls: urls.join("\n"), videoUrl: item.videoFile ? await upload(item.videoFile, "video") : item.videoUrl, coverFile: undefined, galleryFiles: undefined, videoFile: undefined });
      }
