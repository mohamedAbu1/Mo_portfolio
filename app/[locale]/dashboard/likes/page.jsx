...
export default function LikesPage() {
  ...
  useEffect(() => { if (status !== "authenticated") return; const seenAt = new Date().toISOString(); window.localStorage.setItem("admin-likes-seen-at", seenAt); window.dispatchEvent(new window.Event("admin-likes-seen")); fetch("/api/admin/likes").then((r) => r.json()).then((d) => setLikes(d.data || [])); }, [status]);
